...
app.post('/createdb',function(req,res){
    nano.db.create(req.body.dbname,function(err){
        if(err){
            res.send('Error creating the database: '+req.body.dbname);
            return;
        }

        res.send('Database created successfully '+req.body.dbname);
    });
});
...
app.post('/new_contact',function (req, res){
    ...
    db.insert({name:name,phone:phone,crazy:true},phone,function (err,body,header) {
        if(err){
            res.send('Error creating the contact');
            return;
        }
...
app.post('/delete_contact',function(req,res){
    db.get(req.body.phone,function(err,body){
        if(!err){
